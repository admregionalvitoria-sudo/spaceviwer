...
Push-Location $root
try {
    & 'C:\msys64\msys2_shell.cmd' -defterm -here -no-start -ucrt64 -c 'cmake -S native/spaceviwerstream -B cmake-build-spaceviewer -G Ninja -DCMAKE_BUILD_TYPE=Release && cmake --build cmake-build-spaceviewer --target SenaiStream SenaiStreamDisplayCtl test_sunshine -j 4 && cmake-build-spaceviewer/tests/test_sunshine.exe'
    ...
    @{ version = '2.2.11'; builtAt = (Get-Date).ToUniversalTime().ToString('o'); sources = @($sources); binaries = @(
